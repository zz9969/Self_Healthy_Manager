...
void ConstitutionWidget::buildQuestionUI()
{
    ...
    for (int i = 0; i < questions_.size(); ++i) {
        ...
        qLabel->setStyleSheet("font-weight:bold; font-size:15px; margin-top:10px; color:#333;");
        ...
        for (int j = 0; j < options.size(); ++j) {
            ...
            radio->setStyleSheet("font-size:15px; margin-left:20px; padding:2px;");
...
void ConstitutionWidget::showResult(const QJsonObject& data)
{
    ...
    title->setStyleSheet("font-size:22px; font-weight:bold; color:#4CAF50; padding:6px;");
    title->setAlignment(Qt::AlignCenter);
    layout->addWidget(title);

    QLabel* primaryLabel = new QLabel("主要体质: " + primaryType + "质");
    primaryLabel->setStyleSheet("font-size:18px; font-weight:bold; color:#2196F3; padding:4px;");
    ...
    if (!secondaryType.isEmpty()) {
        ...
        secondaryLabel->setStyleSheet("font-size:15px; color:#FF9800; padding:2px;");
    ...
    scoreTitle->setStyleSheet("font-weight:bold; font-size:15px; color:#555;");
    ...
    for (const QString& type : typeKeys) {
        ...
        nameLabel->setStyleSheet("font-size:15px; color:#333;");
    ...
    if (!dietAdvice.isEmpty()) {
        ...
        adviceTitle->setStyleSheet("font-weight:bold; font-size:15px; color:#2E7D32;");
        layout->addWidget(adviceTitle);

        QLabel* adviceLabel = new QLabel(dietAdvice);
        adviceLabel->setWordWrap(true);
        adviceLabel->setStyleSheet("font-size:15px; color:#555; padding:4px; "
