...
public:
    ...
    void clearAuth() {
        QString oldToken = authToken_.isEmpty() ? getAuthToken() : authToken_;
        authToken_.clear();
        previousLogin_.clear();
        QSettings settings("DailyDiet", "Client");
        settings.remove("auth_token");
    }

    void clearAuthAndNotify() {
        clearAuth();
        emit unauthorized();
    }

    void setPreviousLogin(const QString& loginTime) {
        previousLogin_ = loginTime;
    }

    QString getPreviousLogin() const {
        return previousLogin_;
    }
    ...
    void onReplyFinished(QNetworkReply* reply) {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            if (reply->error() == QNetworkReply::AuthenticationRequiredError) {
                QString currentToken = getAuthToken();
                clearAuth();
