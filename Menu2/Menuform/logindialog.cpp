    ...
    , viewmodel_(new LoginViewModel(this))
{
    ...
    connect(ui->pb_tourist, &QPushButton::clicked, this, &LoginDialog::onTouristClicked);
    connect(ui->pb_register, &QPushButton::clicked, this, &LoginDialog::onRegisterClicked);
    connect(ui->pb_register_clear, &QPushButton::clicked, this, &LoginDialog::onClearRegisterClicked);
...
void LoginDialog::onLoginSuccess(const QJsonObject& data)
{
    ...
    ApiClient::getInstance().setPreviousLogin(data.value("previous_login").toString());
...
void LoginDialog::onLoadingChanged()
{
    ...
    ui->pb_tourist->setEnabled(!loading);
}
