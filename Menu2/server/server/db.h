...
public:
    ...
    std::vector<std::vector<std::string>> executeQueryPublic(const std::string& query) {
        std::lock_guard<std::mutex> lock(dbMutex_);
        return executeQuery(query, {});
    }

    std::vector<std::vector<std::string>> executeQueryPublicParams(const std::string& query,
                                                                     const std::vector<std::string>& params) {
        std::lock_guard<std::mutex> lock(dbMutex_);
        return executeQuery(query, params);
    }

    void executeUpdatePublic(const std::string& query, const std::vector<std::string>& params) {
        std::lock_guard<std::mutex> lock(dbMutex_);
        executeUpdate(query, params);
    }

    void fixZeroNutritionRecipes() {
        auto rows = executeQueryPublic(
            "SELECT recipe_id, name, category FROM tcm_recipes WHERE protein = 0 AND fat = 0 AND carbs = 0");
        if (rows.empty()) return;

        std::cout << "[DB] Found " << rows.size() << " recipes with zero nutrition, fixing..." << std::endl;

        for (auto& row : rows) {
            int rid = 0;
            try { rid = std::stoi(row[0]); } catch (...) {}
            std::string name = row[1];

            try {
                NutritionResult result = AiNutritionService::getInstance().analyzeDish(name);

                if (result.calories > 0 || result.protein_g > 0) {
                    executeUpdatePublic(
                        "UPDATE tcm_recipes SET calories = ?, protein = ?, fat = ?, carbs = ? WHERE recipe_id = ?",
                        {std::to_string((int)result.calories), std::to_string(result.protein_g),
                         std::to_string(result.fat_g), std::to_string(result.carbs_g),
                         std::to_string(rid)});
                    std::cout << "[DB] Fixed nutrition for '" << name << "': "
                              << result.calories << "kcal" << std::endl;
                }
            } catch (const std::exception& e) {
                std::cerr << "[DB] Failed to fix nutrition for '" << name << "': " << e.what() << std::endl;
            } catch (...) {
                std::cerr << "[DB] Unknown error fixing nutrition for '" << name << "'" << std::endl;
            }
        }
    }

    std::string getLastLogin(int userId) {
        std::lock_guard<std::mutex> lock(dbMutex_);
        auto rows = executeQuery("SELECT last_login FROM users WHERE user_id = ?", {std::to_string(userId)});
        if (rows.empty() || rows[0][0].empty()) return "";
        return rows[0][0];
    }

    void updateLastLogin(int userId) {
        std::lock_guard<std::mutex> lock(dbMutex_);
        executeUpdate("UPDATE users SET last_login = NOW() WHERE user_id = ?", {std::to_string(userId)});
    }
    ...
    std::string getConstitutionResult(int userId) {
        ...
        std::string testDate = row[10];

        bool expired = false;
        if (testDate.size() >= 10) {
            int y = 0, m = 0, d = 0;
            sscanf_s(testDate.c_str(), "%d-%d-%d", &y, &m, &d);
            struct tm tm_test = {0};
            tm_test.tm_year = y - 1900;
            tm_test.tm_mon = m - 1;
            tm_test.tm_mday = d;
            time_t testTime = mktime(&tm_test);
            time_t now = time(nullptr);
            double diffDays = difftime(now, testTime) / (60.0 * 60.0 * 24.0);
            expired = (diffDays > 30.0);
        }
        ...
        ss << ",\"特禀\":" << row[8] << "}";
        ss << ",\"test_date\":\"" << json::escape(testDate) << "\"";
    ...
    int saveDietRecord(int userId, const std::string& mealType, const std::string& body, const std::string& notes) {
        ...
        if (recipeId <= 0 && !foodName.empty()) {
            auto rows = executeQuery("SELECT recipe_id, calories, protein, fat, carbs FROM tcm_recipes WHERE name = ? LIMIT 1",
                                     {foodName});
            if (!rows.empty()) {
                try { recipeId = std::stoi(rows[0][0]); } catch (...) {}
                if (recipeId > 0) {
                    try { calories = std::stod(rows[0][1]); } catch (...) {}
                }
            }
        }
        ...
        if (recipeId <= 0 && !foodName.empty()) {
            lock.unlock();
            NutritionResult aiResult;
            try {
                aiResult = AiNutritionService::getInstance().analyzeDish(foodName);
            } catch (...) {
                aiResult = NutritionCalculator::defaultEstimation(foodName);
            }
            lock.lock();

            calories = aiResult.calories;
            protein = aiResult.protein_g;
            fat = aiResult.fat_g;
            carbs = aiResult.carbs_g;

            std::string cat = "其他";
            if (foodName.find("粥") != std::string::npos || foodName.find("饭") != std::string::npos) cat = "主食";
            else if (foodName.find("鸡") != std::string::npos || foodName.find("鱼") != std::string::npos
                     || foodName.find("肉") != std::string::npos || foodName.find("虾") != std::string::npos) cat = "肉类";
            else if (foodName.find("菜") != std::string::npos || foodName.find("瓜") != std::string::npos) cat = "蔬菜";
            else if (foodName.find("汤") != std::string::npos) cat = "汤类";
            else if (foodName.find("面") != std::string::npos) cat = "主食";
            else if (foodName.find("蛋") != std::string::npos) cat = "蛋类";
            else if (foodName.find("豆") != std::string::npos) cat = "豆制品";

            std::string ingredientsJson = "[";
            for (size_t i = 0; i < aiResult.ingredients.size(); ++i) {
                if (i > 0) ingredientsJson += ",";
                ingredientsJson += "{\"name\":\"" + json::escape(aiResult.ingredients[i].name)
                                 + "\",\"weight_g\":" + std::to_string((int)aiResult.ingredients[i].weight_g) + "}";
            }
            ingredientsJson += "]";

            recipeId = executeInsert(
                "INSERT INTO tcm_recipes (name, category, calories, protein, fat, carbs, ingredients) VALUES (?, ?, ?, ?, ?, ?, ?)",
                {foodName, cat, std::to_string((int)calories),
                 std::to_string(protein), std::to_string(fat), std::to_string(carbs),
                 ingredientsJson});

            std::cout << "[DB] AI nutrition for '" << foodName << "': "
                      << calories << "kcal, P=" << protein << "g, F=" << fat << "g, C=" << carbs << "g"
                      << (aiResult.fromAI ? " (from AI)" : " (default)") << std::endl;
        } else if (recipeId > 0) {
            auto rows = executeQuery("SELECT protein, fat, carbs FROM tcm_recipes WHERE recipe_id = ?",
                                     {std::to_string(recipeId)});
            if (!rows.empty()) {
                try { protein = std::stod(rows[0][0]); } catch (...) {}
                try { fat = std::stod(rows[0][1]); } catch (...) {}
                try { carbs = std::stod(rows[0][2]); } catch (...) {}
            }
        }

        if (recipeId <= 0) return -1;

        int inputMode = (json::getInt(body, "recipe_id", 0) > 0) ? 1 : 2;

        std::string query = "INSERT INTO diet_records (user_id, meal_type, recipe_id, quantity, calories, protein, fat, carbs, input_mode, record_date) "
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())";
        int id = executeInsert(query, {std::to_string(userId), mealType,
                                       std::to_string(recipeId), std::to_string(quantity),
                                       std::to_string((int)calories),
                                       std::to_string(protein), std::to_string(fat), std::to_string(carbs),
                                       std::to_string(inputMode)});
...
private:
    ...
    void addLastLoginColumn() {
        auto check = executeQuery("SELECT COUNT(*) FROM information_schema.columns "
                                  "WHERE table_schema = DATABASE() AND table_name = 'users' AND column_name = 'last_login'", {});
        if (!check.empty() && std::stoi(check[0][0]) > 0) return;
        executeUpdate("ALTER TABLE users ADD COLUMN last_login DATETIME DEFAULT NULL", {});
        std::cout << "[DB] Added last_login column to users table" << std::endl;
    }
