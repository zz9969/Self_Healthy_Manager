...
HttpResponse UserHandler::handleLogin(const HttpRequest& req) {
    ...
    auto& user = userOpt.value();
    std::string sessionId = DbManager::getInstance().createSession(user.userId);
    std::string previousLogin = DbManager::getInstance().getLastLogin(user.userId);
    DbManager::getInstance().updateLastLogin(user.userId);
    ...
    std::string data = json::buildDataNum({
        ...
        {"previous_login", std::string("\"") + json::escape(previousLogin) + "\""}
    });
...
HttpResponse UserHandler::handleTourist(const HttpRequest& req) {
    ...
    if (userId > 0) {
        ...
        DbManager::getInstance().updateLastLogin(userId);
    } else {
